...
export default Route.extend({
    ...
    actions:
    {
        mailid_data()
        {
            var url = "mailid_data?mailid=" + document.getElementById("mailid").value + "&mailid_choice=" + document.getElementById("mailid_choice").value;
                    ...
                    request.onreadystatechange = function () {
                        if (this.readyState == 4){
                            if(this.status==200)
                            {
                               ...
                               const mailid_data = JSON.parse(val);
                               console.log(val);
                               console.log(mailid_data);
                               const elem=document.getElementById("data");
                               elem.innerHTML="";
                           for(const student of mailid_data)
                           {
