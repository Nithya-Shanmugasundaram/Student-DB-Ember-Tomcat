...
export default Route.extend({
    ...
    actions:
    {
        dept_data()
        {
            var url = "dept_data?dept=" + document.getElementById("dept").value + "&dept_choice=" + document.getElementById("dept_choice").value;
                    ...
                    request.onreadystatechange = function () {
                        if (this.readyState == 4){
                            if(this.status==200)
                            {
                               ...
                               const dept_data = JSON.parse(val);
                               console.log(val);
                               console.log(dept_data);
                               const elem=document.getElementById("data");
                               elem.innerHTML="";
                           for(const student of dept_data)
                           {
